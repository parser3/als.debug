...
@dstop[*o]
$result[^Als/Debug:_stop[$o]]


@dshow[*o]
$result[^Als/Debug:_show[$o]]
...
@_stop[o]
^if($self.can){
	^process[$MAIN:CLASS]{@unhandled_exception[exception^;stack]^#0a^^Als/Debug:_exception[^$exception^;^$stack]}
...
^if($self.can){
	^if(!$self.called){
		...
		$t[^process[$MAIN:CLASS]{@postprocess[result]^#0a^$result[^^Als/Debug:_postprocess[^$result]]}]
		$self.called(true)
	}
...
^if($self.[_show_$o.CLASS_NAME] is junction){
	$result[^Als/Debug:[_show_$o.CLASS_NAME][$o;$indent;$root]]
}{
	$result[^Als/Debug:_show_DEFAULT[$o;$indent;$root]]
}
...
^if($self.called && ($type eq "text/html" || !^type.match[/(xml|json)])){
	$result[^result.match[(^^.*<(?:html|body)[^^>]*>)?][i]{$match.1^self._outer_html[${Als/Debug:console}]}]
}
